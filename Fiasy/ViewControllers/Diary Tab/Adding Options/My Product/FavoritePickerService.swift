//
//  FavoritePickerService.swift
//  Fiasy
//
//  Created by Eugen Lipatov on 7/18/19.
//  Copyright © 2019 Eugen Lipatov. All rights reserved.
//

import UIKit

class FavoritePickerService: NSObject, UINavigationControllerDelegate {
    
    var targetVC: UIViewController?
    
    internal var edit: (() -> Void)?
    ...
    func showPicker() {
        ...
        let edit = UIAlertAction(title: "Редактировать", style: .default,
                             handler: { [weak self] _ in
                                    guard let `self` = self else { return }
                                    self.edit?()
        })
        edit.setValue(#colorLiteral(red: 0.9501664042, green: 0.6013857722, blue: 0.2910895646, alpha: 1), forKey: "titleTextColor")
        refreshAlert.addAction(edit)
