...
class TemplateSearchViewController: UIViewController {
    ...
    func reloadBottomView() {
        ...
        if SwiftEntryKit.isCurrentlyDisplaying() {
            ...
            productPresetsView.fillView(count: UserInfo.sharedInstance.templateProductList.count)
    ...
    private func displayPresets() {
        if UserInfo.sharedInstance.templateProductList.isEmpty {
            SwiftEntryKit.dismiss()
        } else {
            if SwiftEntryKit.isCurrentlyDisplaying() {
                productPresetsView.fillView(count: UserInfo.sharedInstance.templateProductList.count)
...
extension TemplateSearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if products.isEmpty {
            ...
            //if product.measurementUnits.isEmpty {
                ...
                cell.fillSecondCell(indexPath.row == 0, product: products[indexPath.row], delegate: self, UserInfo.sharedInstance.templateProductList)
                return cell
           // }
//            else {
...
extension TemplateSearchViewController: ProductAddingDelegate  {
    ...
    func productSelected(_ item: SecondProduct, state: Bool) {
        ...
        if state {
            UserInfo.sharedInstance.templateProductList.append(filled)
        } else {
            for (index, second) in UserInfo.sharedInstance.templateProductList.enumerated() where second.id == filled.id {
                if UserInfo.sharedInstance.templateProductList.indices.contains(index) {
                    UserInfo.sharedInstance.templateProductList.remove(at: index)
                }
            }
        }
        displayPresets()
...
extension TemplateSearchViewController: SearchProductInteractorOutput {
...
extension TemplateSearchViewController: UITextFieldDelegate {
