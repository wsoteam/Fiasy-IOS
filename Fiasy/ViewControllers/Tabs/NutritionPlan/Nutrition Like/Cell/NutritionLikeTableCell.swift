...
class NutritionLikeTableCell: UITableViewCell {
    ...
    //MARK: - Properties -
    private var delegate: NutritionLikeDelegate?
    ...
    // MARK: - Interface -
    func fillRow(nutrition: NutritionDetail, delegate: NutritionLikeDelegate, _ indexPath: IndexPath) {
        self.delegate = delegate
        self.nutrition = nutrition
        topConstraint.constant = indexPath.row == 0 ? 3.0 : 10.0
        likeButton.setImage(#imageLiteral(resourceName: "someLike"), for: .normal)
    ...
    @IBAction func likeClicked(_ sender: Any) {
        ...
        likeButton.setImage(#imageLiteral(resourceName: "nutrition_favorite_empty"), for: .normal)
        delegate?.removeSomeNutrition(nutrition: nutrition, likeButton)
    }
}
